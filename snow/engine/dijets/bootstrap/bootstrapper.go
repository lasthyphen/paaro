...
func (b *Bootstrapper) process(vtxs ...dijets.Vertex) error {
...
func (b *Bootstrapper) MultiPut(vdr ids.ShortID, requestID uint32, vtxs [][]byte) error {
	...
	processVertices := make([]dijets.Vertex, 1, len(vtxs)) // Process all of the valid vertices in this message
...
func (b *Bootstrapper) ForceAccepted(acceptedContainerIDs []ids.ID) error {
	...
	toProcess := make([]dijets.Vertex, 0, len(pendingContainerIDs))
