...
func GetProcessConfig(v *viper.Viper) (process.Config, error) {
	...
	//
	// build
	// ├── paaro (the binary from compiling the app directory)
	// └── plugins
	//     └── evm
...
func getConsensusConfig(v *viper.Viper) dijets.Parameters {
	return dijets.Parameters{
...
func GetNodeConfig(v *viper.Viper, buildDir string) (node.Config, error) {
	...
	// Genesis Data
	nodeConfig.GenesisBytes, nodeConfig.DjtxAssetID, err = genesis.Genesis(
