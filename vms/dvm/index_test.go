...
var indexEnabledDvmConfig = Config{
...
func TestIndexTransaction_Ordered(t *testing.T) {
	...
	djtxID := genesisTx.ID()
	vm := setupTestVM(t, ctx, baseDBManager, genesisBytes, issuer, indexEnabledDvmConfig)
	defer func() {
...
func setupTestVM(t *testing.T, ctx *snow.Context, baseDBManager manager.Manager, genesisBytes []byte, issuer chan common.Message, config Config) *VM {
	vm := &VM{}
	dvmConfigBytes, err := BuildDvmConfigBytes(config)
	assert.NoError(t, err)
	if err := vm.Initialize(ctx, baseDBManager.NewPrefixDBManager([]byte{1}), genesisBytes, nil, dvmConfigBytes, issuer, []*common.Fx{{
		ID: ids.Empty,
		Fx: &secp256k1fx.Fx{},
	}}); err != nil {
		t.Fatal(err)
