...
func (vm *VM) newExportTx(
	...
	case rules.IsApricotPhase3:
		...
		djtxIns, djtxSigners, err = vm.GetSpendableAVAXWithFee(keys, djtxNeeded, cost, baseFee)
	default:
		var newAvaxNeeded uint64
		newAvaxNeeded, err = math.Add64(djtxNeeded, params.AvalancheAtomicTxFee)
		if err != nil {
			return nil, errOverflowExport
		}
		djtxIns, djtxSigners, err = vm.GetSpendableFunds(keys, vm.ctx.AVAXAssetID, newAvaxNeeded)
	}
	if err != nil {
		return nil, fmt.Errorf("couldn't generate tx inputs/signers: %w", err)
	}
