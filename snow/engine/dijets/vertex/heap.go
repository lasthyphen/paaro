...
type priorityQueue []dijets.Vertex
...
func (vh *maxHeightVertexHeap) Push(vtx dijets.Vertex) bool {
	vtxID := vtx.ID()
...
func (vh *maxHeightVertexHeap) Pop() dijets.Vertex {
	vtx := heap.Pop(&vh.heap).(dijets.Vertex)
