...
import (
	...
	"github.com/lasthyphen/paaro/snow/consensus/dijets/poll"
...
func (t *Transitive) finishBootstrapping() error {
	...
	edge := t.Manager.Edge()
	frontier := make([]dijets.Vertex, 0, len(edge))
...
func (t *Transitive) GetAncestors(vdr ids.ShortID, requestID uint32, vtxID ids.ID) error {
	...
	queue := make([]dijets.Vertex, 1, t.Config.MultiputMaxContainersSent) // for BFS
	queue[0] = vertex
...
// Returns true if [vtx] has been added to consensus (now or previously)
func (t *Transitive) issueFrom(vdr ids.ShortID, vtx dijets.Vertex) (bool, error) {
	issued := true
...
// Assumes we have [vtx].
func (t *Transitive) issue(vtx dijets.Vertex) error {
	vtxID := vtx.ID()
...
func (t *Transitive) GetVtx(vtxID ids.ID) (dijets.Vertex, error) {
