...
func addNodeFlags(fs *flag.FlagSet) {
	...
	// Logging
	fs.String(LogsDirKey, "", "Logging directory for Dijets")
	...
	fs.Int(SnowDijetsNumParentsKey, 5, "Number of vertexes for reference from each new vertex")
	fs.Int(SnowDijetsBatchSizeKey, 30, "Number of operations to batch in each new vertex")
	...
	// Chain Config Dir
	fs.String(ChainConfigDirKey, defaultChainConfigDir, "Chain specific configurations parent directory. Defaults to $HOME/.paaro/configs/chains/")
...
// BuildFlagSet returns a complete set of flags for paaro
