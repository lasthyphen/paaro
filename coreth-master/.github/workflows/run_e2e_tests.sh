...
# Defines the paaro tag to use
# Either uses the same tag as the current branch or uses the default
# Disable matchup in favor of explicit tag
# TODO re-enable matchup when our workflow better supports it.
# if docker_tag_exists $paaro_repo $current_branch; then
#     echo "$paaro_repo:$current_branch exists; using this paaro image to run e2e tests"
#     DIJETS_VERSION=$current_branch
# else
#     echo "$paaro_repo $current_branch does NOT exist; using the default image to run e2e tests"
# fi
...
avalanche_image="avaplatform/paaro:$build_image_id"
