...
import (
	...
	"github.com/lasthyphen/paaro/database/mockdb"
...
func GetDJTXTxFromGenesisTest(genesisBytes []byte, tb testing.TB) *Tx {
...
func GenesisVMWithArgs(tb testing.TB, additionalFxs []*common.Fx, args *BuildGenesisArgs) ([]byte, chan common.Message, *VM, *atomic.Memory) {
	...
	configBytes, err := BuildDvmConfigBytes(Config{IndexTransactions: true})
	if err != nil {
		tb.Fatal("should not have caused error in creating dvm config bytes")
	}
	err = vm.Initialize(
		ctx,
...
func NewTxWithAsset(t *testing.T, genesisBytes []byte, vm *VM, assetName string) *Tx {
	...
	newTx := &Tx{UnsignedTx: &BaseTx{BaseTx: djtx.BaseTx{
		NetworkID:    networkID,
...
func setupIssueTx(t testing.TB) (chan common.Message, *VM, *snow.Context, []*Tx) {
	...
	key := keys[0]
	firstTx := &Tx{UnsignedTx: &BaseTx{BaseTx: djtx.BaseTx{
		NetworkID:    networkID,
		BlockchainID: chainID,
		Ins: []*djtx.TransferableInput{{
			UTXOID: djtx.UTXOID{
	...
	secondTx := &Tx{UnsignedTx: &BaseTx{BaseTx: djtx.BaseTx{
		NetworkID:    networkID,
		BlockchainID: chainID,
		Ins: []*djtx.TransferableInput{{
			UTXOID: djtx.UTXOID{
	...
	return issuer, vm, ctx, []*Tx{djtxTx, firstTx, secondTx}
}
...
func TestTxNotCached(t *testing.T) {
	...
	s.StatusState = djtx.NewStatusState(prefixdb.New(statusStatePrefix, db))
	s.uniqueTxs.Flush()
...
func TestTxVerifyAfterVerifyAncestorTx(t *testing.T) {
	...
	secondTx := issueTxs[2]
	key := keys[0]
	firstTxDescendant := &Tx{UnsignedTx: &BaseTx{BaseTx: djtx.BaseTx{
		NetworkID:    networkID,
		BlockchainID: chainID,
		Ins: []*djtx.TransferableInput{{
			UTXOID: djtx.UTXOID{
