...
if [[ $# -eq 2 ]]; then
    ...
    if [[ ! -d "$coreth_path" ]]; then
        go get "github.com/djt-labs/coreth@$coreth_version"
    fi
else
...
cd "$coreth_path"
go build -ldflags "-X github.com/djt-labs/coreth/plugin/evm.Version=$coreth_version $static_ld_flags" -o "$evm_path" "plugin/"*.go
cd "$DIJETS_PATH"
