...
func TestExportTxSemanticVerifyInvalidFx(t *testing.T) {
	...
	if err := rawTx.UnsignedTx.SemanticVerify(vm, utx.UnsignedTx, []verify.Verifiable{&djtx.TestVerifiable{}}); err == nil {
...
func TestClearForceAcceptedExportTx(t *testing.T) {
	...
	dvmConfigBytes, err := BuildDvmConfigBytes(dvmConfig)
...
// Returns the byte representation of the given DVM config
func BuildDvmConfigBytes(config Config) ([]byte, error) {
