...
import (
	...
	"github.com/djt-labs/paaro/api/admin"
	"github.com/djt-labs/paaro/api/auth"
	"github.com/djt-labs/paaro/api/health"
	"github.com/djt-labs/paaro/api/info"
	"github.com/djt-labs/paaro/api/keystore"
	"github.com/djt-labs/paaro/api/metrics"
	...
	"github.com/djt-labs/paaro/indexer"
	"github.com/djt-labs/paaro/ipcs"
	"github.com/djt-labs/paaro/network"
	"github.com/djt-labs/paaro/network/dialer"
	...
	"github.com/djt-labs/paaro/vms/rpcchainvm"
	"github.com/djt-labs/paaro/vms/secp256k1fx"

	ipcsapi "github.com/djt-labs/paaro/api/ipcs"
)
...
// Node is an instance of an Dijets node.
type Node struct {
	Log        logging.Logger
...
// DVM, Simple Payments DAG, Simple Payments Chain, and Platform VM
// Assumes n.DBManager, n.vdrs all initialized (non-nil)
func (n *Node) initChainManager(djtxAssetID ids.ID) error {
	createDVMTx, err := genesis.VMGenesis(n.Config.GenesisBytes, dvm.ID)
	if err != nil {
		return err
	}
	xChainID := createDVMTx.ID()
	...
	// Register the VMs that Dijets supports
	errs := wrappers.Errs{}
	errs.Add(
		...
		n.vmManager.RegisterFactory(dvm.ID, &dvm.Factory{
			TxFee:            n.Config.TxFee,
			CreateAssetTxFee: n.Config.CreateAssetTxFee,
		}),
