...
import (
	...
	"github.com/lasthyphen/paaro/vms/components/index"
...
// VM implements the dijets.DAGVM interface
type VM struct {
	Factory
	metrics
	djtx.AddressManager
...
// Initialize implements the dijets.DAGVM interface
func (vm *VM) Initialize(
	...
	if len(configBytes) > 0 {
		if err := json.Unmarshal(configBytes, &dvmConfig); err != nil {
			return err
		}
		ctx.Log.Info("VM config initialized %+v", dvmConfig)
	}
	...
	vm.AtomicUTXOManager = djtx.NewAtomicUTXOManager(ctx.SharedMemory, vm.codec)
	...
	if dvmConfig.IndexTransactions {
		vm.ctx.Log.Info("address transaction indexing is enabled")
		vm.addressTxsIndexer, err = index.NewIndexer(vm.db, vm.ctx.Log, ctx.Namespace, ctx.Metrics, dvmConfig.IndexAllowIncomplete)
		if err != nil {
		...
		vm.addressTxsIndexer, err = index.NewNoIndexer(vm.db, dvmConfig.IndexAllowIncomplete)
		if err != nil {
...
// Shutdown implements the dijets.DAGVM interface
...
// Get implements the dijets.DAGVM interface
func (vm *VM) Version() (string, error) {
	return version.Current.String(), nil
}

// CreateHandlers implements the dijets.DAGVM interface
func (vm *VM) CreateHandlers() (map[string]*common.HTTPHandler, error) {
	...
	// name this service "dvm"
	if err := rpcServer.RegisterService(&Service{vm: vm}, "dvm"); err != nil {
		return nil, err
	}
...
func (vm *VM) CreateStaticHandlers() (map[string]*common.HTTPHandler, error) {
	...
	}, newServer.RegisterService(staticService, "dvm")
}

// Pending implements the dijets.DAGVM interface
...
// Parse implements the dijets.DAGVM interface
...
func (vm *VM) getAllUTXOs(addrs ids.ShortSet) ([]*djtx.UTXO, error) {
...
func (vm *VM) getAllUniqueAddressUTXOs(addr ids.ShortID, seen *ids.Set, utxos *[]*djtx.UTXO) error {
	lastIndex := ids.Empty
...
func (vm *VM) verifyTransferOfUTXO(tx UnsignedTx, in *djtx.TransferableInput, cred verify.Verifiable, utxo *djtx.UTXO) error {
...
func (vm *VM) verifyTransfer(tx UnsignedTx, in *djtx.TransferableInput, cred verify.Verifiable) error {
