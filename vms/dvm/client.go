...
// NewClient returns an DVM client for interacting with dvm [chain]
func NewClient(uri, chain string, requestTimeout time.Duration) *Client {
	return &Client{
		requester: rpc.NewEndpointRequester(uri, fmt.Sprintf("/ext/%s", constants.ChainAliasPrefix+chain), "dvm", requestTimeout),
	}
}
...
// ImportDJTX sends an import transaction to import funds from [sourceChain] and
// returns the ID of the newly created transaction
// This is a deprecated name for Import
func (c *Client) ImportDJTX(user api.UserPass, to, sourceChain string) (ids.ID, error) {
...
func (c *Client) ExportDJTX(
	...
	return c.Export(user, from, changeAddr, amount, to, "DJTX")
}
