...
func AddTest(t *testing.T, factory Factory) {
	...
	if !avl.Finalized() {
		t.Fatalf("An empty dijets instance is not finalized")
	...
	case err != nil:
		t.Fatal(err)
	case avl.Finalized():
		t.Fatalf("A non-empty dijets instance is finalized")
...
func VotingTest(t *testing.T, factory Factory) {
	...
	case err != nil:
		t.Fatal(err)
	case avl.Finalized():
		t.Fatalf("An dijets instance finalized too early")
	...
	case err != nil:
		t.Fatal(err)
	case !avl.Finalized():
		t.Fatalf("An dijets instance finalized too late")
	case !ids.UnsortedEquals([]ids.ID{vtx1.IDV}, avl.Preferences().List()):
		t.Fatalf("Initial frontier failed to be set")
